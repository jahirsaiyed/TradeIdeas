...
class TradeIdeasKafkaResourceIT {
    ...
    void setup() {
        ...
        TradeIdeasKafkaResource kafkaResource = new TradeIdeasKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/trade-ideas-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/trade-ideas-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
